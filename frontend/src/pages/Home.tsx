    ...
    <div className="font-sans text-wellness-text bg-wellness-bg selection:bg-calm-sage/20 selection:text-calm-sage overflow-x-hidden">
            ...
            <div className="p-6 md:p-8">
              ...
              {loginStatus === 'idle' && (
                <>
                  <div className="flex items-center justify-between mb-8 relative">
                    ...
                    <h2 className="text-[1.35rem] font-bold text-[#1A1A1A] font-serif tracking-tight">Log into your account</h2>
                    <button 
                      onClick={handleCloseLogin}
                  ...
                  <div className="space-y-4">
                    ...
                    <button 
                      ...
                      className="w-full flex items-center justify-center gap-3 bg-white border border-slate-200 hover:bg-slate-50 hover:border-slate-300 text-[#1A1A1A] text-[1.05rem] font-medium py-4 rounded-full transition-all relative active:scale-[0.98]"
                    >
                  ...
                  <p className="mt-8 text-xs text-center text-slate-500 leading-relaxed px-4 opacity-80">
                    By tapping Continue or logging into an existing Manas360 account, you agree to our <a href="#" className="text-[#0A3A78] font-bold hover:underline">Terms</a> and acknowledge that you have read our <a href="#" className="text-[#0A3A78] font-bold hover:underline">Privacy Policy</a>.
                  </p>
                </>
              )}
              ...
              {loginStatus === 'success' && (
                <div className="flex flex-col items-center justify-center py-8 text-center animate-fade-in">
                   ...
                   <h2 className="text-[2rem] font-serif font-bold text-[#0A3A78] mb-4">Welcome to Manas360</h2>
                   <p className="text-[#2E3A48] text-lg mb-8 leading-relaxed max-w-xs mx-auto">
                     You have successfully registered! <br/>
                     <span className="font-bold text-[#1E59FF]">A confirmation email has been sent to your inbox.</span>
                   </p>
                   <button 
                     onClick={handleCloseLogin}
                     className="w-full px-10 py-4 rounded-full bg-gradient-to-r from-[#1E59FF] to-[#004BCE] text-white text-lg font-bold shadow-[0_10px_25px_-5px_rgba(30,89,255,0.4)] hover:shadow-xl hover:-translate-y-1 transition-all"
                   >
            ...
            {loginStatus === 'idle' && (
              <div className="bg-[#F8FAFC] py-5 text-center border-t border-slate-100">
                 <p className="text-[0.95rem] text-slate-600">
                   Don't have an account? <a href="#" className="text-[#0A3A78] font-bold hover:underline">Sign up</a>
                 </p>
              </div>
            )}
          </div>
      ...
      >
        ...
        <nav className="relative z-50 flex items-center justify-between px-6 md:px-10 py-6 max-w-[1400px] mx-auto w-full">
          <div className="font-serif text-[1.6rem] md:text-[1.8rem] font-bold text-[#0A3A78] tracking-tight cursor-pointer hover:opacity-90 transition-opacity drop-shadow-sm z-50">
            Manas360
          </div>

          <div className="hidden lg:flex items-center gap-6 xl:gap-10 font-medium text-[#0A3A78] text-[1.05rem]">
          ...
          <div className="hidden lg:flex items-center gap-4 xl:gap-6">
            <button 
              ...
              className="text-[#0A3A78] font-bold hover:text-[#1E59FF] transition-colors px-3 py-2 text-[1.05rem]"
            >
              Log In
            </button>
            <button className="text-[#0A3A78] text-2xl hover:scale-110 transition-transform p-2 drop-shadow-sm">🛒</button>
            <button 
              onClick={() => navigate('/assessment')}
              className="px-10 py-3.5 rounded-full bg-gradient-to-r from-[#0052CC] to-[#2684FF] text-white font-bold shadow-[0_4px_14px_0_rgba(0,118,255,0.39)] hover:shadow-[0_8px_25px_rgba(0,118,255,0.35)] hover:-translate-y-0.5 hover:brightness-105 transition-all duration-300 ease-out tracking-wide text-[1.05rem]"
            >
              Get Started
            </button>
          </div>
          ...
          <div className="lg:hidden flex items-center gap-4 z-50">
             <button 
                onClick={() => setIsLoginOpen(true)}
                className="text-[#0A3A78] font-bold text-[0.95rem] hover:text-[#1E59FF] transition-colors"
              >
                Log In
              </button>
              <button className="text-[#0A3A78] text-xl p-1">🛒</button>
              <button 
                onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)}
                className="text-[#0A3A78] text-3xl ml-1 focus:outline-none transition-transform duration-300"
        ...
        >
            ...
            <div className="flex-1 overflow-y-auto pt-28 px-6 pb-12 relative z-10 flex flex-col">
                <div className="space-y-8 flex-1">
                    <div className="space-y-4">
                        <h4 className="font-serif text-[#0A3A78] text-2xl font-bold opacity-90">Solutions</h4>
                        <div className="pl-4 border-l-2 border-blue-50 space-y-4">
                            <a href="#" className="block text-lg font-medium text-slate-600 hover:text-[#1E59FF]">Wellness Seekers</a>
                            <a href="#" className="block text-lg font-medium text-slate-600 hover:text-[#1E59FF]">Providers</a>
                            <a href="#" className="block text-lg font-medium text-slate-600 hover:text-[#1E59FF]">Corporates</a>
                            <a href="#" className="block text-lg font-medium text-slate-600 hover:text-[#1E59FF]">Education</a>
                        </div>
                    </div>

                    <div className="space-y-4">
                        <h4 className="font-serif text-[#0A3A78] text-2xl font-bold opacity-90">Resources</h4>
                        <div className="pl-4 border-l-2 border-blue-50 space-y-4">
                            <a href="#" className="block text-lg font-medium text-slate-600 hover:text-[#1E59FF]">Sound Therapy</a>
                            <a href="#" className="block text-lg font-medium text-slate-600 hover:text-[#1E59FF]">Digital Pet Therapy</a>
                            <a href="#" className="block text-lg font-medium text-slate-600 hover:text-[#1E59FF]">Virtual Bot</a>
                        </div>
                    </div>

                    <div>
                        <a href="#" className="font-serif text-[#0A3A78] text-2xl font-bold opacity-90 block mb-4">About Us</a>
                    </div>
                </div>
                ...
                <div className="mt-8">
                     <button 
                      ...
                      className="w-full py-5 rounded-full bg-gradient-to-r from-[#0052CC] to-[#2684FF] text-white text-xl font-bold shadow-[0_10px_30px_-5px_rgba(30,89,255,0.4)] active:scale-[0.98] transition-transform"
                    >
                      Get Started
                    </button>
        ...
        <div className="relative z-20 flex-1 flex flex-col justify-center items-center text-center px-4 max-w-5xl mx-auto pb-24">
          <div className="relative z-10 p-6 md:p-8 rounded-[3rem] md:bg-gradient-to-b md:from-white/5 md:to-white/0 md:backdrop-blur-[2px]">
            <h1 className="font-serif text-[2.8rem] md:text-[clamp(2.8rem,6vw,5.5rem)] font-normal text-[#0A3A78] leading-[1.1] mb-6 md:mb-8 drop-shadow-lg text-balance tracking-tight">
              A Holistic 360° Journey to Greater Mental Wellness
            </h1>
            
            <p className="text-[1.1rem] md:text-[1.4rem] text-[#2E3A48] leading-[1.6] max-w-4xl mx-auto mb-10 md:mb-14 font-medium opacity-90 text-balance drop-shadow-sm md:text-[#1a3b5c]">
      ...
      <section className="py-24 px-6 relative">
        ...
        <div className="max-w-[1280px] mx-auto relative z-10">
          <h2 className="reveal-on-scroll font-serif text-[2.8rem] md:text-[3.2rem] text-[#0A3A78] mb-16 text-center tracking-tight">
            Our Solutions
          </h2>
        ...
        <div className="max-w-[1280px] mx-auto grid grid-cols-1 md:grid-cols-2 gap-12 lg:gap-16 relative z-10">
          ...
          <div className="reveal-on-scroll group bg-gradient-to-br from-white to-[#EBF5FF] rounded-[40px] p-10 md:p-14 flex flex-col justify-between hover:shadow-[0_30px_60px_-15px_rgba(14,165,233,0.15)] transition-all duration-500 border border-white shadow-[0_20px_40px_-10px_rgba(0,0,0,0.03)]">
            <div>
              ...
              <h3 className="font-serif text-[2.4rem] text-[#0A3A78] mb-6 leading-tight">Virtual Therapist Bot</h3>
            ...
            <button className="self-start px-10 py-5 rounded-full bg-white text-[#1E59FF] font-bold text-lg border-2 border-[#1E59FF]/10 hover:border-[#1E59FF] hover:bg-[#1E59FF] hover:text-white transition-all duration-300 shadow-sm hover:shadow-lg">
              Start Chatting Now
            </button>
          </div>
          ...
          <div className="reveal-on-scroll group bg-gradient-to-br from-white to-[#FFFBEB] rounded-[40px] p-10 md:p-14 flex flex-col justify-between hover:shadow-[0_30px_60px_-15px_rgba(245,158,11,0.15)] transition-all duration-500 border border-white shadow-[0_20px_40px_-10px_rgba(0,0,0,0.03)] [transition-delay:100ms]">
            <div>
              ...
              <h3 className="font-serif text-[2.4rem] text-[#0A3A78] mb-6 leading-tight">Digital Pet Therapy</h3>
              <p className="text-[#475569] text-[1.15rem] mb-12 leading-relaxed font-light">
        ...
        <div className="max-w-[1280px] mx-auto reveal-on-scroll">
          <div className="bg-gradient-to-br from-[#DDEFFD] via-[#F0F8FF] to-white rounded-[50px] p-12 md:p-24 flex flex-col md:flex-row items-center gap-16 border border-white shadow-[0_40px_80px_-20px_rgba(14,165,233,0.15)] group relative overflow-hidden">
            ...
            <div className="flex-1 relative z-10">
              <span className="inline-block px-6 py-2 rounded-full bg-white text-[#0A3A78] text-xs font-bold tracking-[0.2em] uppercase mb-10 shadow-[0_4px_10px_rgba(0,0,0,0.05)] border border-blue-50">
                Featured Collection
              </span>
              <h2 className="font-serif text-[3rem] md:text-[4rem] text-[#0A3A78] leading-[1.1] mb-8 tracking-tight">
              ...
              <p className="text-[1.25rem] text-[#475569] leading-relaxed mb-12 font-light max-w-xl">
               ...
               <button className="flex items-center gap-3 text-[#1E59FF] font-bold text-xl hover:gap-5 transition-all group-hover:text-[#004BCE]">
            ...
            <div className="flex-1 flex justify-center relative z-10">
              <div className="relative">
                ...
                <div className="w-56 h-56 md:w-72 md:h-72 rounded-full bg-gradient-to-br from-[#1E59FF] to-[#004BCE] flex items-center justify-center shadow-[0_30px_60px_-10px_rgba(30,89,255,0.4)] hover:scale-105 transition-transform duration-500 cursor-pointer group hover:shadow-[0_40px_80px_-10px_rgba(30,89,255,0.5)]">
      ...
      <section className="py-32 px-6 bg-gradient-to-b from-[#F8FAFC] to-white text-center border-b border-[#E0F2FE]/50">
        <div className="max-w-5xl mx-auto reveal-on-scroll">
          <h2 className="font-serif text-[2.8rem] md:text-[3.5rem] text-[#0A3A78] mb-10 tracking-tight leading-tight">
      ...
      <section className="py-32 px-6 bg-white text-center relative overflow-hidden">
        ...
        <div className="max-w-4xl mx-auto relative z-10 reveal-on-scroll">
          <h2 className="font-serif text-[3.2rem] md:text-[4.5rem] text-[#0A3A78] mb-10 leading-[1.1] text-balance tracking-tight">
          ...
          <p className="text-[1.35rem] text-[#475569] mb-14 font-light leading-relaxed">
            ...
            <br />
            <span className="font-semibold text-[#1E59FF]">First assessment is free.</span>
          </p>
          
          <div className="flex flex-col items-center gap-6">
            <button 
              onClick={() => navigate('/assessment')}
              className="px-20 py-7 rounded-full bg-gradient-to-r from-[#0052CC] to-[#2684FF] text-white text-xl font-bold shadow-[0_20px_40px_-10px_rgba(30,89,255,0.3)] hover:shadow-[0_30px_60px_-10px_rgba(30,89,255,0.4)] hover:-translate-y-1 hover:brightness-105 transition-all duration-300 ring-4 ring-blue-500/10"
            >
              Start Your Journey
            </button>
      ...
      <footer className="bg-[#F8FAFC] pt-24 pb-12 px-6 border-t border-slate-100">
        <div className="max-w-[1280px] mx-auto text-center">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-12 md:gap-8 mb-20 text-left">
            <div>
              <h4 className="font-bold text-[#0A3A78] mb-8 text-lg">Manas360</h4>
              <ul className="space-y-4 text-[0.95rem] text-slate-500 font-medium">
                <li className="hover:text-[#1E59FF] cursor-pointer transition-colors">About Us</li>
                <li className="hover:text-[#1E59FF] cursor-pointer transition-colors">Careers</li>
                <li className="hover:text-[#1E59FF] cursor-pointer transition-colors">Press</li>
                <li className="hover:text-[#1E59FF] cursor-pointer transition-colors">Blog</li>
              </ul>
            </div>
            <div>
              <h4 className="font-bold text-[#0A3A78] mb-8 text-lg">Solutions</h4>
              <ul className="space-y-4 text-[0.95rem] text-slate-500 font-medium">
                ...
                <li className="hover:text-[#1E59FF] cursor-pointer transition-colors">For Schools</li>
                <li className="hover:text-[#1E59FF] cursor-pointer transition-colors">For Therapists</li>
              </ul>
            </div>
            <div>
              <h4 className="font-bold text-[#0A3A78] mb-8 text-lg">Support</h4>
              <ul className="space-y-4 text-[0.95rem] text-slate-500 font-medium">
                ...
                <li className="hover:text-[#1E59FF] cursor-pointer transition-colors">Accessibility</li>
              </ul>
            </div>
             <div>
              <h4 className="font-bold text-[#0A3A78] mb-8 text-lg">Legal</h4>
              <ul className="space-y-4 text-[0.95rem] text-slate-500 font-medium">
                ...
                <li className="hover:text-[#1E59FF] cursor-pointer transition-colors">Cookies</li>
              </ul>
            </div>
          </div>
...
const DropdownItem: React.FC<{ children: React.ReactNode }> = ({ children }) => (
  <a href="#" className="block px-5 py-3 text-[0.95rem] font-medium text-[#2E3A48] hover:bg-sky-50/80 hover:text-[#0A3A78] rounded-xl transition-all duration-200 hover:pl-6">
    {children}
  </a>
);
...
const SolutionCard: React.FC<{ title: string; subtitle?: string; icon: string; gradient: string; delay?: string }> = ({ title, subtitle, icon, gradient, delay }) => (
  ...
  >
    ...
    <h3 className="font-serif text-[1.6rem] font-bold text-[#0A3A78] mb-2 leading-tight">{title}</h3>
