...
const setAnswer = (questionId: string, optionIndex: number) => {
 ...
 const handleFinish = async () => {
  ...
  const hasMissing = answersPayload.some((answer) =>
    Number.isNaN(answer.optionIndex)
  );
  ...
  try {
    const answerScores = answersPayload.map((item) => item.optionIndex);

    const response = await http.post('/v1/patients/assessments', {
      type: 'PHQ-9',
    ...
    const isCritical =
      String(result?.severityLevel || '').toLowerCase() === 'severe';
...
>
  {submitting
    ...
    : isComplete
      ...
      : `Answer ${Object.keys(answers).length}/${questions.length}`}
</button>
        </div>
